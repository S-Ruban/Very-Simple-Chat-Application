...
public class Client_Machine extends Frame implements ActionListener
{
	
	Frame f = new Frame();	// creating a new Java AWT Frame object
	Label head = new Label("Client Machine");
	...
	Client_Machine(String address, int port) throws IOException
	{
		f.setBounds(900, 300, 490, 417);
		f.setLayout(null);
		f.setVisible(true);
		...
		try
		{
			  ...
			  chat.setText("Server is online. Say Hi!\n\n");
			  in = new DataInputStream(socket.getInputStream());  
		      out = new DataOutputStream(socket.getOutputStream());
		      while(true)
		      {
		    	  String data = in.readUTF();	// receiving texts from the client
			      chat.append("Server (" + form.format(new Date()) + ") : " + data + "\n");	// updating the chat
		      }
		}
	...
	public void actionPerformed(ActionEvent ae)
	{
		try
		{
			if(!text.getText().trim().equals(""))	// if the textbox is not empty
			{
				...
				chat.append("Client (" + form.format(new Date()) + ") : " + text.getText() + "\n");	// updating the chat
			}
		}
		catch (IOException | NullPointerException ex)
		{
			chat.append("Server is not online to receive your messages.\n\n");
		}
		finally
		{
	...
	{
		new Client_Machine(args[0], 42069);	// accepting first (and only argument) of program as IP address and connecting to port 42069
	}
